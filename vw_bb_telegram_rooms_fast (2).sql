-- BB ONLY / TELEGRAM ROOMS FINAL
-- Source: public.vw_bb_orders_all_v2
...
with base as (
  select to_jsonb(o) as j from public.vw_bb_orders_all_v2 o
), prepared as (
  select
    j,
...
create view public.vw_bb_telegram_delivery_queue_fast as select *, 'PENDING_REVIEW_OR_SEND'::text as queue_status from public.vw_bb_telegram_delivery_source_fast where not is_sent and source_time >= (((case when (now() at time zone 'Asia/Bangkok')::time >= time '14:00:00' then (now() at time zone 'Asia/Bangkok')::date else (now() at time zone 'Asia/Bangkok')::date - 1 end) + time '14:00:00') at time zone 'Asia/Bangkok');
create view public.vw_bb_telegram_delivery_today_fast as select *, 'CURRENT_CUTOFF_SEND_READY'::text as queue_status from public.vw_bb_telegram_delivery_source_fast where not is_sent and source_time >= (((case when (now() at time zone 'Asia/Bangkok')::time >= time '14:00:00' then (now() at time zone 'Asia/Bangkok')::date else (now() at time zone 'Asia/Bangkok')::date - 1 end) + time '14:00:00') at time zone 'Asia/Bangkok');
create view public.vw_bb_telegram_delivery_yesterday_after_14_fast as select *, 'FROM_YESTERDAY_14_TO_TODAY_14_SEND_READY'::text as queue_status from public.vw_bb_telegram_delivery_source_fast where not is_sent and source_time >= (((now() at time zone 'Asia/Bangkok')::date - 1) + time '14:00:00') at time zone 'Asia/Bangkok' and source_time < (((now() at time zone 'Asia/Bangkok')::date) + time '14:00:00') at time zone 'Asia/Bangkok';

comment on view public.vw_bb_telegram_delivery_source_fast is 'BB Telegram source from vw_bb_orders_all_v2; review and out-of-stock rows are retained.';
comment on view public.vw_bb_telegram_delivery_queue_fast is 'BB Telegram waiting queue; sent rows excluded; rolling cutoff starts after 14:00:00 Asia/Bangkok.';
comment on view public.vw_bb_telegram_delivery_today_fast is 'BB Telegram today room by Facebook order time in Asia/Bangkok.';
comment on view public.vw_bb_telegram_delivery_yesterday_after_14_fast is 'BB Telegram rolling room from yesterday 14:00 to today 14:00 Asia/Bangkok.';

commit;

-- Verify after running:
-- select count(*) from public.vw_bb_telegram_delivery_queue_fast;
-- select order_number, order_time_display, product_for_delivery, quantity_from_order_table, master_stock_notice, is_sent from public.vw_bb_telegram_delivery_queue_fast order by source_time desc limit 20;
-- select mapping_status, count(*) from public.vw_bb_telegram_delivery_source_fast group by mapping_status;
-- select product_stock_state, count(*) from public.vw_bb_telegram_delivery_source_fast group by product_stock_state;
-- select telegram_status, is_sent, count(*) from public.vw_bb_telegram_delivery_source_fast group by telegram_status, is_sent;
-- select * from public.vw_bb_telegram_delivery_source_fast where source_time is null or telegram_message_dynamic is null;
