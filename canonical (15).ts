...
export async function readAlienReview(search = ''): Promise<AlienReviewItem[]> {
  ...
  const [ordersResult, masterResult, aliasResult, inventoryResult] = await Promise.all([
  ...
  return (ordersResult.data ?? []).map((order: any) => {
    ...
    const masterDisplay = String(master?.master_display_for_packer ?? master?.display_for_packer ?? order.master_display_for_packer ?? '').trim();
