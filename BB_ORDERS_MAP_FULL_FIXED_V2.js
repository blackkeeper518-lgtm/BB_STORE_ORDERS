 ...
 *
 * หน้าที่:
 * 1) แปลง chat_timeline ที่เป็น Array / JSON string / ข้อความเดี่ยว ให้เป็น Array เดียวกัน
 * 2) อ่านสินค้าในข้อความบอทและข้อความคน
 * 3) ไม่ให้ CHECK_SKU, ตรวจสอบ หรือ SKU ว่าง ทับ SKU จริง
 * 4) รักษายอดเงินจริงจาก expected_cod เป็นหลัก ห้ามคำนวณจากราคา
 */
...
const EMOJI_MAP_BY_KEYWORD = [
  ...
  [/GOLD|ทอง/iu, '🟡'], [/RED|แดง/iu, '🟥'], [/BLUE|ฟ้า|น้ำเงิน/iu, '🟦'],
  [/PURPLE|ม่วง/iu, '🟪'], [/BLACK|ดำ/iu, '⬛'], [/WHITE|ขาว/iu, '⬜'],
  [/GREEN|เขียว/iu, '🟩'], [/MENTHOL|MINT|เย็น|มิ้นต์|มินต์/iu, '❄️'],
  [/APPLE|แอปเปิ้ล/iu, '🍎'], [/ORANGE|ส้ม/iu, '🍊'], [/LEMON|มะนาว/iu, '🍋'],
  [/PEACH|พีช/iu, '🍑'], [/COCONUT|มะพร้าว/iu, '🥥'],
];

const ALIASES = [
  [/คาโร่?แตงโม|คาวาโร่แตงโม/iu, 'CAVALLO_WATERMELON'],
  [/คาวาเขียว|คาวาโร่เขียว/iu, 'CAVALLO_GREEN'],
  [/คาวาแดง|คาวาโร่แดง/iu, 'CAVALLO_RED'],
  [/ม่อนเขียว|มอนเขียว|ม่อนกรีน|มอนกรีน/iu, 'MOND_GREEN'],
  [/ม่อนทอง|มอนทอง/iu, 'MOND_GOLD'],
  [/เวสเขียว/iu, 'VESS_GREEN'],
  [/เวสแดง/iu, 'VESS_RED'],
  [/ซีวอสแดง|ชีวอสแดง/iu, 'SEVIOS_RED'],
  [/ซีวอสเขียว|ชีวอสเขียว/iu, 'SEVIOS_GREEN'],
  [/โอเอสแตงโม|OS\s*WATERMELON/iu, 'OS_WATERMELON'],
  [/แตงโม/iu, 'OS_WATERMELON'], // Default แตงโมทั่วไป
];
...
function normalizeSku(value) {
  ...
  const match = source.match(/\b[A-Z][A-Z0-9]*(?:_[A-Z0-9]+)+\b/);
  const sku = (match ? match[0] : source)
    .replace(/[ .-]+/g, '_')
    .replace(/[^A-Z0-9_]/g, '')
    .replace(/_+/g, '_')
    .replace(/^_|_$/g, '');
  if (!sku || sku === 'CHECK_SKU' || sku === 'ตรวจสอบ') return '';
  return sku;
}
...
function add(found, sku, qty, source) {
  const cleanSku = normalizeSku(sku);
  const n = quantity(qty) || 1;
  if (!cleanSku || n <= 0) return;
  const key = `${cleanSku}|${n}`;
  if (!found.has(key)) found.set(key, { sku: cleanSku, qty: String(n), source });
}

function extractProducts(raw, source) {
  const value = text(raw).replace(/[□]/g, ' ');
  const found = new Map();

  // 1. อ่านจาก Bot / สรุปยอด
  const botSection = value.match(/รายการสินค้า\s*[:：]?([\s\S]*)/iu);
  if (botSection) {
    const body = botSection[1].split(/🚚\s*ขนส่ง|การขนส่ง|จัดส่งโดย|ค่าตัว|รวมส่ง/iu)[0];
    const botSku = /\b([A-Z][A-Z0-9_]*(?:\s+[A-Z][A-Z0-9_]*)?)\s*(?:\([^\n|]*\))?\s*(?:\||[-–—]|x|×)?\s*(?:จำนวน\s*)?(\d+(?:\.\d+)?)/giu;
    let match;
    while ((match = botSku.exec(body))) add(found, match[1], match[2], `${source}:bot`);
  }

  // 2. แอดมิน/ลูกค้า: SKU | จำนวน หรือ SKU x จำนวน
  const humanSku = /\b([A-Z][A-Z0-9_]*(?:\s+[A-Z][A-Z0-9_]*)?)\s*(?:\||x|×)\s*(?:จำนวน\s*)?(\d+(?:\.\d+)?)/giu;
  let human;
  while ((human = humanSku.exec(value))) add(found, human[1], human[2], `${source}:human`);

  // 3. Alias ภาษาไทย: อ่านจำนวนจากบรรทัดเดียวกับ alias เท่านั้น
...
// ✅ 2. ฟังก์ชันแก้เรื่องการเลือก Emoji ให้ถูกต้องครบ 100%
function getProductEmoji(sku, rowEmoji) {
  ...
  if (candidate && candidate !== '📦' && /\p{Extended_Pictographic}/u.test(candidate)) return candidate;
  for (const [pattern, emoji] of EMOJI_MAP_BY_KEYWORD) if (pattern.test(cleanSku)) return emoji;
  return '📦';
}

function formatProduct(item, row) {
  const master = PRODUCT_MASTER[item.sku];
  const emoji = getProductEmoji(item.sku, row.extracted_emoji || row.emoji);

  if (master) {
    return `${emoji} ${master.sku}(${master.thName}) ${item.qty} คอต`;
  }
  
  const thName = row.th_name || row.product_name;
  const thLabel = thName && thName !== 'ตรวจสอบ' ? `(${thName})` : '';
  return `${emoji} ${item.sku}${thLabel} ${item.qty} คอต`;
}

function chooseProduct(row, timeline) {
  const summary = [row.sniper_x_text_clean, row.clean_text]
    .map(text)
    .find((value) => /(?:COD|ยอดรวม|ยอด COD|รายการสินค้า|ชื่อ-นามสกุล|ที่อยู่จัดส่ง)/iu.test(value)
      && /(?:\d+\s*คอต|รายการสินค้า|COD|ยอด COD)/iu.test(value)) || '';
  const latestTimelineSummary = [...timeline].reverse().find((value) => /(?:\[เพจ:|รายการสินค้า\s*:|COD\s*[:：])/iu.test(value)) || '';
  const rawTexts = [
    // แหล่งจริงอันดับ 1: ตัดจาก clean_text แล้ว ห้ามเปลี่ยนข้อความ
    ['single_cleaned_products', row.single_cleaned_products],
    // แหล่งยืนยันอันดับ 2: ก้อนสรุปออเดอร์จริงทั้งก้อน
    ['order_summary', summary],
    // แหล่งสำรองอันดับ 3: raw_text สินค้า
    ['raw_text', row.raw_text],
    // ใช้ตรวจเฉพาะข้อความยืนยันล่าสุด ไม่อ่านแชทเก่าทั้งหมด
    ['chat_timeline_confirmed', latestTimelineSummary],
  ...
  return { products, best: formatted.join('\n'), candidates: formatted };
}

const output = items.map((item) => {
  const row = item.json || {};
  const timeline = normalizeTimeline(row.chat_timeline);
  const product = chooseProduct(row, timeline);
  const expectedCod = row.expected_cod ?? row.raw_cod_amount ?? row.cod_amount ?? null;
  const realSku = product.products[0]?.sku || normalizeSku(row.extracted_sku) || normalizeSku(row.sku) || '';

  return {
    json: {
      ...row,
      chat_timeline: timeline,
      chat_timeline_count: timeline.length,
      normalized_chat_timeline: timeline,
      clean_text: row.clean_text,
      extracted_sku: product.products.map((p) => p.sku).join('\n') || null,
      sku: product.products.map((p) => p.sku).join('\n') || null,
      extracted_qty: product.products.map((p) => p.qty).join('\n') || text(row.extracted_qty || row.quantity) || null,
      product_quantity_lines: product.products.map((p) => p.qty).join('\n') || null,
      ...
      mapping_status: product.products.length ? 'MATCHED' : 'REVIEW',
