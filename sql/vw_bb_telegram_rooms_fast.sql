...
create view public.vw_bb_telegram_delivery_queue_fast as select *, 'PENDING_REVIEW_OR_SEND'::text as queue_status from public.vw_bb_telegram_delivery_source_fast where not is_sent;
create view public.vw_bb_telegram_delivery_today_fast as select *, 'TODAY_SEND_READY'::text as queue_status from public.vw_bb_telegram_delivery_source_fast where not is_sent and source_date_bkk = (now() at time zone 'Asia/Bangkok')::date;
...
comment on view public.vw_bb_telegram_delivery_queue_fast is 'BB Telegram waiting queue; sent rows excluded.';
