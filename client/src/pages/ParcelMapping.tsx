...
export default function ParcelMapping() {
  ...
  const effectiveTracking = selectedOrder ? (tracking || selectedDraft?.tracking || "") : "";
  const message = selectedOrder ? [
    ...
    `ขนส่ง: ${carrier || selectedDraft?.carrier || "ไม่ระบุ"}`,
  ...
  useEffect(() => {
    ...
    setTracking(drafts[selectedOrder.order_number]?.tracking || "");
    setCarrier(drafts[selectedOrder.order_number]?.carrier || "FLASH EXPRESS");
  }, [selectedNumber]);
  ...
  return <div className="min-h-[calc(100vh-2rem)] bg-[#09090b] text-white"><div className="mx-auto max-w-[1500px] space-y-5 p-3 sm:p-5 lg:p-7">
    ...
    <footer className="flex flex-wrap items-center justify-between gap-3 px-2 text-[11px] text-slate-500"><span className="flex items-center gap-2"><ExternalLink className="h-3.5 w-3.5" />ลูกค้าเช็คสถานะได้จากเว็บหน้าบ้านของร้าน</span><span>ขั้นต่อไป: บันทึกเลขพัสดุลงห้องกลางและต่อช่องแชทจริง</span></footer>
  </div></div>;
}
